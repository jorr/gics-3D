...
export default class PointCommand extends Command {
  ...
  createItem(params) {
    ...
    // point(<segment>,mult) - a point lying at mult*length along the length of the segment, starting from the left-hand endpoint
      ...
      this.item = new Point({
        x: params[0].p1.x + params[1]*params[0].len,
        y: params[0].p1.y + params[1]*params[0].len,
        z: params[0].p1.z + params[1]*params[0].len
      });
    }
    //point(<point>,dx,dy,dz) - a point distanced at the vector(dx,dy,dz) from the given point
      ...
      this.item = new Point({
        ...
        y: params[0].y + params[2],
        z: params[0].z + params[3]
      });
    }
